...
public class Hanoi {
	...
	public void moveOneRing(int from, int to) {
		/* TODO */
		//Pops out from stack[0]/stack[from] and pushes into stack[1]/stack[to]
	...
	public void movePile(int n, int from, int to, int via) {
		/* TODO */
		if(n==1) {
			moveOneRing(from,to); //Moves number/ring from [0] to [1]
		}else {
			//Moves number/ring from [0] to [2] through [1]
			movePile(n-1, from,via,to); 
			moveOneRing(from,to);
			//Moves back number/ring from [2] to [1] through [0]

...
public class BinarySearchTree {
	...
	private class BSTNode { // private class to hold a tree node
		...
		// Part 1: complete
		/**
		 * This method performs in-order traversal by printing out the numbers in a tree
		 * in the correct order using inOrderTraversalPrint.
		 */
		public void inOrderTraversalPrint() {
			// Checks if left child exists for the root node, if it does then
			// inOrderTraversalPrint is applied to the left child.
			if (this.getLeftChild() != null) {
				// Applies inOrderTraversal to the left child with left child
				// being the root(Recursive)
				this.getLeftChild().inOrderTraversalPrint();
			}
			// Print the value of root node once the leaf node is reached (No more left
			// child nodes)
			System.out.println(this.getValue());

			// Checks whether right child exists for the root node, if it does then
			// a recursive function is used to apply the same function to the right child
			if (this.getRightChild() != null) {
				// Applies inOrderTraversal to the right child with right child
				// being the root(Recursive)
		...
		// Part 2: complete
		/**
		 * This method is used to implement inorder traversal using doubly linked lists.
		 * 
		 * @param dl Doubly linked list that will be use to store inOrderTraversal
		 *           values
		 */
		public void inOrderTraversal(DLinkedList dl) {
			// Checks if left child exists for the root node, if it does then
			// inOrderTraversal is applied to the left child.
			if (this.getLeftChild() != null) {
				// Applies inOrderTraversal to the left child with left child
				// being the root(Recursive)
				this.getLeftChild().inOrderTraversal(dl);
			}
			// Add root value to the tail of the doubly linked list
	...
	// Part 2: complete
	/**
	 * This method is used to create a doubly linked list and store values using
	 * the inOrderTraversal method.
	 * 
	 * @return Returns doubly linked list
	 */
	public DLinkedList<Integer> inOrderTraversal() {
		// Create a doubly linked list
		DLinkedList<Integer> dll = new DLinkedList<>();
		// Checks if root node is null or not
		if (this.rootNode != null) {
			// Passing the doubly linked list to the inOrderTraversal method 
			// which is in the BST node class
			rootNode.inOrderTraversal(dll);
		}
		return dll; // Return the doubly linked list
	}

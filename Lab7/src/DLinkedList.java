...
public class DLinkedList {
	...
	public void insertionSort() {
		...
		if (headNode != null) {
			// Checks if node next to head exists
			if (headNode.getNextNode() != null) {
				// Node stores the address of node next to headNode
				Node nextToHead = headNode.getNextNode();

				for (int i = 1; i < this.size(); i++) {
					// Gets value stored in the node next to head and stores it in an
					// int to be used later for insertion sort, similar to int cur = arr[i];
					// in the implementation of array
					int curr = nextToHead.getValue();
					// Counter which stores index of node before i, which again will be used later
					// to
					// move the nth element to the left for insertion sort
					int j = i - 1;

					Node test = nextToHead;
					// Go backwards to find the correct place to have the current node
					// Once the correct position is found insert it
					while (j >= 0 && test.getPrevNode().getValue() > curr) {
						// Sets value of test to the value stored in the previous node of
						// test
						test.value = test.getPrevNode().getValue();
//						int a = delete(test.getPrevNode());
						// Sets the value of the previous node of test to the value stored
						// in current
						test.getPrevNode().setValue(curr);
//						insertAfter(test,a);

						// Sets test to prevNode of test and decrements j counter
						test = test.getPrevNode();
						j--;
					}
					// Checks if node nextToHead exists
					if (nextToHead.getNextNode() != null) {
						// Sets next nodes reference to nextToHead
						nextToHead = nextToHead.nextNode;
					}

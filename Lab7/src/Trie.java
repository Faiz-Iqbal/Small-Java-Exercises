...
public class Trie {
	...
	private class TrieNode { // This is the class that represents a node in the trie
		...
		private boolean isValidEnd = false; // We need to know if this node represents the 
											//	end point of a a valid word
		...
		public int countAllWords() {
			...
			// stored in the trie.
			int numberOfWords = 0; //Counter for number of words
			/* For loop goes through each of the 26 letters in the trie and checks if there
			 * are any subnodes for that character, if there are then the method countAllWords
			 * is called again whose return value is stored in the numberOfWords.
			 */
			for(int i=0; i < subnodes.length; i++) {
				//Checks to see if there are any subnodes for the character
				if(subnodes[i] != null) {
					//Recursive function is applied where countAllWords is run on the next character
					// and int value stored numberOfWords 
					int charsInWord = subnodes[i].countAllWords();
					numberOfWords += charsInWord;
					//When we reach node which has a valid end(makes a complete word), then
					// the numberOfWords is incremented by 1
					if(subnodes[i].isValidEnd == true) {
						return numberOfWords+=1; //Increments by 1
					}
				}
			}
